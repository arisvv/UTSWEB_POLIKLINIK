    ...
    <div class="form-group mx-sm-3 mb-2">
        ...
        <select class="form-control" name="obat">
            ...
            while ($obat_data = mysqli_fetch_array($obat_query)) {
                $selected = isset($_POST['obat']) && $_POST['obat'] == $obat_data['id'] ? 'selected' : '';
            ?>
                <option value="<?php echo $obat_data['id'] ?>" <?php echo $selected ?>><?php echo $obat_data['nama_obat'] ?></option>
            <?php
            }
            ?>
        </select>
...
if (isset($_POST['simpan'])) {
    $selected_obat = isset($_POST['obat']) ? $_POST['obat'] : '';
    if (isset($_POST['id'])) {
